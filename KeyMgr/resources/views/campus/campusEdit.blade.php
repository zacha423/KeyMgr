    ...
    <section>
        ...
        <form method="POST" action="{{ route('campus.update', ['campus' =>$campus['id']]) }}" class="mt-6 space-y-6">
            ...
            <div>
                ...
                <x-text-input id="city" name="state" type="text" class="mt-1 block w-full" :value="old('city', $campus['city'])" required autofocus autocomplete="city" />
                <x-input-error class="mt-2" :messages="$errors->get('city')" />
            </div>

            <div>
                <x-input-label for="streetAddress" :value="__('Street')" />
                <x-text-input id="streetAddress" name="state" type="text" class="mt-1 block w-full" :value="old('streetAddress', $campus['streetAddress'])" required autofocus autocomplete="streetAddress" />
                <x-input-error class="mt-2" :messages="$errors->get('streetAddress')" />
            </div>

            <div>
                <x-input-label for="postalCode" :value="__('Zip')" />
                <x-text-input id="postalCode" name="state" type="text" class="mt-1 block w-full" :value="old('postalCode', $campus['postalCode'])" required autofocus autocomplete="postalCode" />
            ...
            <div class="flex items-center gap-4">
                ...
                @if (session('status') === 'campus-updated')
                    <p
                        x-data="{ show: true }"
                        x-show="show"
                        x-transition
